...
export function KanbanBoard() {
  ...
  const filteredData = useMemo(() => {
    ...
    return {
      ...
      lists: data.lists.map((list) => ({
        ...list,
        tasks: list.tasks.filter((task) => selectedTags.every((selectedTag) => task.tags?.includes(selectedTag))),
      })),
    }
  }, [data, selectedTags])
  ...
  const handleListDrop = (e: React.DragEvent, dropIndex: number) => {
    const type = e.dataTransfer.getData("type")
    if (type === "list" && draggedListIndex !== null && draggedListIndex !== dropIndex) {
      e.preventDefault()
      e.stopPropagation()
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-6 pt-6 pb-2 border-b flex-shrink-0">
        <div className="flex items-center gap-2 mb-2">
          <Button variant="outline" size="sm" onClick={() => setShowTagFilter(!showTagFilter)} className="gap-2">
                ...
                <Badge
                  ...
                  style={
                    isSelected
                      ...
                      : { backgroundColor: "transparent", color: bgColor, borderColor: bgColor }
                  }
            ...
            {collapsedLists.map((list) => (
              ...
              >
                <div className="flex flex-col items-start">
                  ...
                  <span className="text-xs text-muted-foreground">{list.tasks.length} tasks</span>
                </div>
              </Button>
            ))}
          </div>
      ...
      <div className="flex-shrink-0 border-t bg-background p-4">
        {isAddingList ? (
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" variant="outline" onClick={() => setIsAddingList(false)}>
                    <X className="h-4 w-4" />
                  </Button>
                </div>
              </CardContent>
          ...
          <div className="max-w-md mx-auto">
            <Button variant="outline" className="w-full bg-transparent" onClick={() => setIsAddingList(true)}>
